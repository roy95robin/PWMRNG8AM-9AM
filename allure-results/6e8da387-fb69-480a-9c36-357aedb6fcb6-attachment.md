...
Call log:
  - waiting for getByText(/Aaliyah/i).first()
          ...
          - generic [ref=e174]:
            ...
            - generic [ref=e178]:
              ...
              - listbox [ref=e181]:
                ...
                - option "Nguyen Van A" [ref=e184] [cursor=pointer]:
                  - generic [ref=e185]: Nguyen Van A
                - option "Nguyen A" [ref=e186] [cursor=pointer]:
                  - generic [ref=e187]: Nguyen A
...
```ts
  ...
  37 | 
  38 |   await page.locator('input[placeholder="Type for hints..."]').fill('A');
> 39 |   await page.getByText(/Aaliyah/i).first().click();
     |                                            ^ Error: locator.click: Test timeout of 30000ms exceeded.
  40 | 
  41 |   const usernameInput = page.locator('div').filter({ hasText: /Username/i }).locator('input').first();
  42 |   await usernameInput.fill(uniqueUsername);
  43 | 
  44 |   await page.locator('div.oxd-select-wrapper').nth(1).click();
  45 |   await page.locator('.oxd-select-dropdown').getByText('Enabled').click();
  46 | 
  47 |   const passwordInput = page.locator('div').filter({ hasText: /Password/i }).locator('input[type="password"]').first();
  48 |   const confirmPasswordInput = page.locator('div').filter({ hasText: /Confirm Password/i }).locator('input[type="password"]').first();
  49 |   await passwordInput.fill(password);
  50 |   await confirmPasswordInput.fill(password);
  51 | 
  52 |   await page.getByRole('button', { name: 'Save' }).click();
  53 | 
  54 |   await expect(page.locator('.oxd-toast')).toContainText(/success/i, { timeout: 20000 });
  55 | 
  56 |   await page.getByPlaceholder('Search').fill(uniqueUsername);
  57 |   await page.getByRole('button', { name: /search/i }).click();
  58 | 
  59 |   await expect(page.locator('div.oxd-table-card')).toContainText(uniqueUsername, { timeout: 20000 });
  60 | });
  61 | 
```